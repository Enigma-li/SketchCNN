...
def collect_vis_img(logit_d, logit_n, logit_c, logit_f, npr, normal, depth, shape_mask, ds, gt_field, cline_mask, fm,
    ...
    train_fcfm_proto = tf.summary.image('train_confidence_mp', logit_c, hyper_params['nbDispImg'])
    ...
    train_cline_mask_inv_proto = tf.summary.image('train_clmask_inv', cline_mask, hyper_params['nbDispImg'])
...
def collect_vis_img_val(logit_d, logit_n, logit_c, logit_f, npr, normal, depth, shape_mask, ds, gt_field, cline_mask,
    ...
    val_fcfm_proto = tf.summary.image('val_confidence_map', logit_c, hyper_params['nbDispImg'])
    ...
    val_cline_mask_inv_proto = tf.summary.image('val_clmask_inv', cline_mask, hyper_params['nbDispImg'])
...
def train_procedure(net, train_records, reg_weight):
        ...
        for gpu_id in range(nb_gpus):
            with tf.device('gpu:%d' % gpu_id):
                with tf.name_scope('tower_%s' % gpu_id) as _:
                    ...
                    cur_trainable_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='SASMFGeoNet')
    ...
    with tf.name_scope('solve') as _:
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='.*SASMFGeoNet.*')
    ...
    train_diff_ds_proto = tf.summary.scalar('Training_DepthSampleL2Loss', averaged_losses[3])
    train_diff_reg_proto = tf.summary.scalar('Training_RegL2Loss', averaged_losses[4])
    train_diff_reald_proto = tf.summary.scalar('Training_RealDLoss', averaged_losses[5])
    train_diff_realn_proto = tf.summary.scalar('Training_RealNLoss', averaged_losses[6])
    train_diff_omega_proto = tf.summary.scalar('Training_OmegaLoss', averaged_losses[7])
...
def train_net():
    ...
    train_records = [os.path.join(hyper_params['dbTrain'], item) for item in train_data_records if
                     item.find('train') != -1]
    train_summary, train_step, train_loss = train_procedure(net, train_records, reg_weight_value)
    ...
    valid_ds_loss_proto = tf.summary.scalar('Validating_DepthSampleL2Loss', valid_ds_loss)
    ...
    valid_omega_loss_proto = tf.summary.scalar('Validating_OmegaLoss', valid_omega_loss)
